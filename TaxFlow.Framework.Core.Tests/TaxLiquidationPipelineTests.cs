...
public class TaxLiquidationPipelineTests
{
    ...
    {
        var assetType = AssetType.Create("Grouped Income", null, LiquidationMode.Grouped);
        ...
        var asset1 = TaxableAsset.Create(assetType, new Collection<ExtendedAttribute>
        {
            ExtendedAttribute.Create("AnnualGlobalIncome", "300000", AttributeDataType.Number, true)
        });

        var asset2 = TaxableAsset.Create(assetType, new Collection<ExtendedAttribute>
        {
            ExtendedAttribute.Create("AnnualGlobalIncome", "300000", AttributeDataType.Number, true)
        });

        var result = TaxLiquidationPipeline.Evaluate(new[] { asset1, asset2 }, new TaxEngineOptions { IncludeRuleResults = false });
    ...
    {
        var assetType = AssetType.Create("Solo", null, LiquidationMode.Individual);
        ...
        var first = TaxableAsset.Create(assetType, new Collection<ExtendedAttribute>
        {
            ExtendedAttribute.Create("Base", "100", AttributeDataType.Number, true)
        });

        var second = TaxableAsset.Create(assetType, new Collection<ExtendedAttribute>
        {
            ExtendedAttribute.Create("Base", "200", AttributeDataType.Number, true)
        });

        var result = TaxLiquidationPipeline.Evaluate(new[] { first, second }, new TaxEngineOptions { IncludeRuleResults = false });
